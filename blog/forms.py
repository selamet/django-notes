...
class IletisimForm(forms.Form):
    isim = forms.CharField(widget=forms.TextInput(attrs={'class': 'form-control'}), max_length=50, label='isim',
                           required=False)
...
class BlogForm(forms.ModelForm):
    class Meta:
        model = Blog
        fields = ['title', 'image', 'content', 'yayin_taslak',
                  'kategoriler']  # Blog modelindeki hangi alanları ile çalışacaksın
...
class PostSorugForm(forms.Form):
    ...
    search = forms.CharField(required=False, max_length=500,
                             widget=forms.TextInput(attrs={'placeholder': 'Bir şeyler arayınız',
                                                           'class': 'form-control'}))
